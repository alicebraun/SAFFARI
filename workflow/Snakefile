        ...
        resources: memory="180G"
      	shell:
      		...
      		python polyfun/polyfun.py \
          		...
          		--sumstats {input.gwas} \
  	   			--allow-missing \
          		--ref-ld-chr /home/braun/pgcd0522/finemapping/finemapping_smk-main/baselineLF2.2.UKB{params.ldscores} \
      			--w-ld-chr /home/braun/pgcd0522/finemapping/finemapping_smk-main/baselineLF2.2.UKB{params.weights}
      		"""
...
rule run_polyfun_susie:
 	...
 	params:
 	 	chrom = lambda wildcards: snp_tables[ss].loc[wildcards.snp]['chrom_orig'],
		pos = lambda wildcards: snp_tables[ss].loc[wildcards.snp]['BP'],
		start = lambda wildcards: snp_tables[ss].loc[wildcards.snp]['range.left'],
		end = lambda wildcards: snp_tables[ss].loc[wildcards.snp]['range.right'],
#		Neff = lambda wilcards: sample_size[ss].loc[wildcards.N]
 	conda: "envs/polyfun.yml"
	resources: mem_mb= 32000
 	shell:
 		"""
 		python polyfun/finemapper.py \
 		--geno  /gpfs/work5/0/pgcdac/DWFV2CJb8Piv_0116_pgc_data/HRC_reference.r1-1/pop_EUR/HRC.r1-1.EGA.GRCh37.chr{params.chrom}.impute.plink.EUR \
...
#		Neff = lambda wilcards: sample_size[ss].loc[wildcards.N]
	...
	resources: mem_mb= 32000
  	shell:
  		...
  		--end {params.end} \
  		--method finemap \
 		--finemap-exe /home/braun/pgcd0522/finemapping/finemap_v1.4.2_x86_64/finemap_v1.4.2_x86_64 \
  		--max-num-causal 5 \
  		--out {output.output} \
  		--allow-missing
 		"""
